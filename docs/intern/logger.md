...
public class BusReservationServiceImpl implements BusReservationService {
    ...
    private Logger log = LoggerFactory.getLogger(BusReservationServiceImpl.class);
    ...
    public List<TripDto> addTrip(TripDto tripDto) {
        ...
        if (sourceStop != null) {
            Stop transitStop = getStop(tripDto.getTransitCode());
            if (transitStop != null) {
                    ...
                    if (!sourceStop.getCode().equalsIgnoreCase(destinationStop.getCode())) {
                        ...
                        if (agency != null) {
                            ...
                            if (bus != null) {
                                ...
                                log.info(" transitStop.getName() =============================> " + transitStop.getName());
                                String reverse = transitStop.getName();
                                String temp = "";
                                if(reverse.contains(" => ")){
                                    temp = reverse.replace(" => ", " >= ");
                                }
                                StringBuilder sb = new StringBuilder(temp);
                                String reverseTransit = sb.reverse().toString();
                                log.info(" transitStop.getName() =============================> " + reverseTransit);

                                Stop reversedTransitStop = new Stop();
                                reversedTransitStop.setId(transitStop.getId());
                                reversedTransitStop.setName(reverseTransit);
                                reversedTransitStop.setDetail("fly to the moon");

//                                transitStop.setName(reverseTransit);
                                log.info(" reverse of transitStop =============================> " + reversedTransitStop.getName());

                                Trip froTrip = new Trip()
                                        .setSourceStop(destinationStop)
                                        .setDestStop(sourceStop)
                                        .setTransit(reversedTransitStop)
                                        .setAgency(agency)
                                        .setBus(bus)
                                        .setJourneyTime(tripDto.getJourneyTime())
                                        .setFare(tripDto.getFare());
            ...
            throw exception(STOP, ENTITY_NOT_FOUND, tripDto.getTransitCode());
        }
...
- `@SpringBootApplication`
- `private Logger log = LoggerFactory.getLogger(BusReservationServiceImpl.class);`
- deploy the `application.properties`

```ruby
#logging pattern for file
logging.pattern.file= "%d{yyyy-MM-dd HH:mm:ss} [%thread] %-5level %logger{36} - %msg%n"
logging.file.name=/Users/Git/Desktop/api.log
```

![](img/2021-01-10-01-42-20.png)
